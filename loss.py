...
class FocalLoss(_Loss):
    ...
    def __init__(self,
        ...
        self.gamma = gamma
        self.alpha = alpha
        if isinstance(self.alpha,(float,int)): self.alpha = torch.Tensor([alpha,1-alpha]).to(device)
        if isinstance(self.alpha,list): self.alpha = torch.FloatTensor(alpha).to(device)
        self.reduction = reduction
