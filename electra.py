...
from transformers import (ElectraForTokenClassification, ElectraTokenizer, ElectraConfig,
                          AdamW, get_linear_schedule_with_warmup, ElectraModel)
...
class PunctuationPredictionModel(ElectraForTokenClassification):

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,valid_ids=None,attention_mask_label=None,loss_type='poly_focal'):
        ...
        if labels is not None:
            if loss_type == 'dice':
              loss_fct = DiceLoss(with_logits=True, smooth=1, ohem_ratio=0.0,
                                alpha=0.1, square_denominator=True,
                                index_label_position=True, reduction="mean")
            elif loss_type == 'focal':
              loss_fct = FocalLoss(gamma=0.2, reduction="mean")
            elif loss_type == 'poly':
              loss_fct = PolyLoss(reduction='mean', epsilon=2.0)
            elif loss_type == 'poly_focal':
              loss_fct = PolyFocalLoss(reduction='mean', epsilon=3.0, gamma=3, alpha=[0.1, 0.5, 0.45, 0.8, 0.6, 0.6, 0.8, 0.5, 0.5, 0.5])
            elif loss_type == 'combo':
              loss_fct = ComboLoss(reduction='mean', smooth=1, alpha=0.75, ce_ratio=0.5)
            else:
...
def main():
    ...
    special_tokens = ['<url_obj>', '<email_obj>']
    ...
    # Prepare model
    config = ElectraConfig.from_pretrained(args.bert_model, num_labels=num_labels, finetuning_task=args.task_name)
    ...
    if args.do_train:
        train_features = convert_examples_to_features(
            train_examples, label_list, args.max_seq_length, tokenizer, noise_prob=0.3, mode='train', add_noise=False)
        ...
        # train_data = pickle.load(open('punctuation/large/news/train_dataset.pkl', 'rb'))
        ...
        for epoch in range(int(start_epoch), int(args.num_train_epochs)):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                # eval_data = pickle.load(open('punctuation/large/news/test_dataset.pkl', 'rb'))
