...
class PunctuationPredictionModel(BertForTokenClassification):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,valid_ids=None,attention_mask_label=None,loss_type='ce'):
        ...
        if labels is not None:
            if loss_type == 'dice':
              ...
              loss_fct = FocalLoss(gamma=2, reduction="mean")
            elif loss_type == 'poly':
              loss_fct = PolyLoss(reduction='mean', epsilon=1.0)
            elif loss_type == 'poly_focal':
              loss_fct = PolyFocalLoss(reduction='mean', epsilon=2.0, gamma=0.2)
            else:
...
def main():
    ...
    special_tokens = ['<email_obj>', '<url_obj>']
    ...
    if args.do_train:
        ...
        # train_data = pickle.load(open('punctuation/large/news/train_dataset_bert4news.pkl', 'rb'))
        if args.local_rank == -1:
        ...
        for epoch in range(int(start_epoch), int(args.num_train_epochs)):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                # eval_data = pickle.load(open('punctuation/large/news/test_dataset_bert4news.pkl', 'rb'))
