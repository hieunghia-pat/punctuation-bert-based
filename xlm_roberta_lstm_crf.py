...
from transformers import (XLMRobertaForTokenClassification, XLMRobertaTokenizer, XLMRobertaConfig,
                          AdamW, get_linear_schedule_with_warmup, XLMRobertaModel)
...
class PunctuationPredictionModel(XLMRobertaForTokenClassification):
    def __init__(self, config):
        super(PunctuationPredictionModel, self).__init__(config=config)
        self.crf = CRF(config.num_labels, batch_first=True)
        self.lstm = nn.LSTM(input_size=config.hidden_size, hidden_size=config.hidden_size//2, num_layers=2, batch_first=True, bidirectional=True)
...
def main():
    ...
    # Prepare model
    config = XLMRobertaConfig.from_pretrained(args.bert_model, num_labels=num_labels, finetuning_task=args.task_name)
    ...
    if args.do_train:
        ...
        train_data = pickle.load(open('punctuation/large/news/train_dataset_xlm.pkl', 'rb'))
        if args.local_rank == -1:
        ...
        for epoch in range(int(start_epoch), int(args.num_train_epochs)):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                eval_data = pickle.load(open('punctuation/large/news/test_dataset_xlm.pkl', 'rb'))
                ...
                for input_ids, input_mask, segment_ids, label_ids,valid_ids,l_mask in eval_dataloader:
                    ...
                    with torch.no_grad():
                        sequence_tags = model(input_ids, segment_ids, input_mask,valid_ids=valid_ids,attention_mask_label=l_mask)
